...
- [ ] Test policy is coherent across AGENTS.md and CLAUDE.md: `TEST_POLICY=required` has a `TARGETED_TEST_CMD`; `TARGETED_TEST_CMD` is not present when policy is off/absent; and `TEST_ELIGIBILITY[]` is not empty/inert when a policy is set. Every `title:` matcher is a valid extended regex (an invalid one is a dead matcher at runtime — catch it here), and `TEST_ELIGIBILITY[]` carries no `label:` matcher when the chosen task source has no labels (local task-list / MEMORY.md): a `label:`-only `except` set would otherwise denylist-enable every task. Use `title:` matchers there
- [ ] The test-discipline wiring (`resolve_test_policy`, the per-task reset of `TARGETED_TEST_FILE` / `FROZEN_TARGETED_TEST_TARGET` / `TEST_GATE_ACTIVE`, RED-before-GREEN sub-phase) is present for the **selected task source**, not only the GitHub-issue path. The `FROZEN_TARGETED_TEST_TARGET=""` reset per task is mandatory in every variant — a multi-task run would otherwise carry one task's RED-confirmed target into the next task's GREEN reruns
...
TEST_POLICY, TEST_ELIGIBILITY[], TARGETED_TEST_CMD,
