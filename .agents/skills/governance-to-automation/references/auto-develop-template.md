...
run_targeted_test_gate() {  # <logfile> [expect_red|expect_green]
  ...
  # bracket expr literal.) Sanitizing here is required regardless of bash -c vs eval.
  ...
  { echo "=== ($mode) $cmd ==="; bash -c "$cmd" 2>&1; } > "$logfile" || rc=$?   # bash -c (not eval); $target is allowlist-sanitized above
